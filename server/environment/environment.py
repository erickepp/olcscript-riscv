...
class Environment:
    ...
    def save_variable(self, ast, id, symbol, line, col, declaration_type):
        ...
        if declaration_type == 'var':
            ...
            ast.set_symbols(id, 'Variable', symbol.type.name.lower(), self.id, symbol.position, line)
        elif declaration_type == 'const':
            self.constants[id] = symbol
            ast.set_symbols(id, 'Constante', symbol.type.name.lower(), self.id, symbol.position, line)
